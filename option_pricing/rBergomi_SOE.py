...
class rBergomi_SOE:
    def __init__(self, M, T, params, P, Lambda, Omega, cores, loop, rand_seed):
        ...
        self.minue = np.zeros((self.M,))
        node_mat = self.Lambda[:, np.newaxis] + self.Lambda[np.newaxis, :]
        weight_mat = self.Omega[:, np.newaxis] * self.Omega[np.newaxis, :]
        for i in range(1, self.M + 1):
            self.minue[i-1] = np.sum(weight_mat/node_mat * (1 - np.exp(-node_mat * self.grid[i])))
        
        self.minue *= self.nu**2 * self.H
        
        

        # compute covariance matrix 
        self.cov = Cov_SOE(self.Nexp, self.Lambda, self.tau)
    ...
    def generate_V_chunk(self, chunk_size):        
        ...
        for i in range(self.M): 
            sample = np.random.multivariate_normal(np.zeros(self.Nexp +1), self.cov, chunk_size)
            W[:, i] = sample[:, 0]
            hist = self.coef * hist + sample[:, 1:]
            mul[:, i] = np.sum(self.Omega.reshape(1, -1) * hist, axis = 1)       

        V = np.exp(self.nu * np.sqrt(2*self.H) * mul - self.minue.reshape(1, -1)) 
