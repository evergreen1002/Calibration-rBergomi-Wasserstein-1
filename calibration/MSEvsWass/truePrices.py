...
def trueP(m_param, config):
    ...
    return true_S, true_price_surface


config = Config()
m_param = tf.constant([[0.09, 0.07, -0.9, 1.9],
                       [0.04, 0.07, -0.9, 1.9],
                       [0.09, 0.02, -0.9, 1.9],
                       [0.09, 0.07, -0.7, 2.2]], dtype = config.datype)
for i in range(1,2):
    true_S, true_price_surface = trueP(m_param[i, :], config)
    true_S = tf.sort(true_S, 0)
    np.save(f"Calibration/MSEvsWass/truePrices/true_S_{i}.npy", true_S.numpy())
    np.save(f"Calibration/MSEvsWass/truePrices/true_price_surface_{i}.npy", true_price_surface.numpy())
