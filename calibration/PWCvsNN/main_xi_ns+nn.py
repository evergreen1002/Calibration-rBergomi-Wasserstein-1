...
def train(config, true_S, type):    
    ...
    param_history.append([model.m_param.H.numpy(), model.m_param.rho.numpy(), model.m_param.nu.numpy(),
                          model.m_param.beta_0.numpy(), model.m_param.beta_1.numpy(), model.m_param.beta_2.numpy(),
                          model.m_param.ttau.numpy(), model.m_param.alpha.numpy()])
    ...
    for step in range(1, config.num_iterations+1):       
        ...
        nu = model.m_param.nu
        beta_0 = model.m_param.beta_0
        beta_1 = model.m_param.beta_1
        beta_2 = model.m_param.beta_2
        ttau = model.m_param.ttau
        alpha = model.m_param.alpha
        param_history.append([H.numpy(), rho.numpy(), nu.numpy(), beta_0.numpy(), 
                              beta_1.numpy(), beta_2.numpy(), ttau.numpy(), alpha.numpy()])        
        
        np.save("FinalResult/loss_history_1_ns+nn.npy", np.array(loss_history))
        np.save("FinalResult/param_history_1_ns+nn.npy", np.array(param_history))
        np.save("FinalResult/xi_history_1_ns+nn.npy", np.array(xi_history))
...
if __name__ == "__main__":
    ...
    train(config, true_S, type = "ns+nn")
